...
class FpgaAsyncRequest:
    ...
    def __str__(self):
        ...
        self.request, self.request_id, self.time_tx, str(self.reply), len(self.informs))
...
class FpgaClient(CallbackClient):
    ...
    def _nb_request(self, request, inform_cb=None, reply_cb=None, *args):
        ...
        self.callback_request(msg=Message.request(request, *args), reply_cb=self._nb_replycb,
                              inform_cb=self._nb_informcb, user_data=request_id)
    ...
    def tap_start(self, tap_dev, device, mac, ip, port):
        ...
        if len(tap_dev) > 8:
            raise RuntimeError(
                "Tap device identifier must be shorter than 9 characters. You specified %s for device %s." % (
                tap_dev, device))
        ...
        if reply.arguments[0] == 'ok':
            ...
            raise RuntimeError(
                "Failure starting tap device %s with mac %s, %s:%s" % (device, mac_str, ip_str, port_str))
    ...
    def get_10gbe_core_details(self, dev_name):
        ...
        arp = []
        for i in range(256):
            arp.append(((port_dump[0x3000 + i * 8 + 2] << 40) + (port_dump[0x3000 + i * 8 + 3] << 32) + (
                        port_dump[0x3000 + i * 8 + 4] << 24) + (port_dump[0x3000 + i * 8 + 5] << 16) + (
                                    port_dump[0x3000 + i * 8 + 6] << 8) + port_dump[0x3000 + i * 8 + 7]))
        rv['arp'] = arp
    ...
    def get_snap(self, dev_name, brams, man_trig=False, man_valid=False, wait_period=1, offset=-1,
                 circular_capture=False, word_mult=1):
        ...
        self._logger.warn(
            "Deprecation warning: get_snap is to be deprecated. Please replace your design's %s block with a 'snapshot' block and use the snapshot_get function instead." % dev_name)
        ...
        while not (done and (offset >= 0 or circular_capture)) and (
    ...
    def snapshot_get(self, dev_name, man_trig=False, man_valid=False, wait_period=1, offset=-1, circular_capture=False,
        ...
        if (bram_size != self.read_uint(dev_name + '_status') & 0x7fffffff) or bram_size == 0:
            # if address is still changing, then the snap block didn't finish capturing. we return empty.
            raise RuntimeError(
                "A snap block logic error occurred or it didn't finish capturing in the allotted %2.2f seconds. Reported %i bytes captured." % (
                wait_period, bram_size))
            bram_dmp['length'] = 0
            bram_dmp['offset'] = 0
            bram_size = 0
